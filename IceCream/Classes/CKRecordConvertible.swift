    ...
    for prop in properties {
      ...
      if prop.isArray {
        ...
        case .object:
          ...
          let referenceArray: [CKRecord.Reference] = dynamicList(prop.name).compactMap { object in
            guard let primaryKey = object.objectSchema.primaryKeyProperty?.name else { return nil }
            ...
            case .string:
              if let primaryValueString = object[primaryKey] as? String, let obj = object as? CKRecordConvertible, !obj.isDeleted {
                ...
                return CKRecord.Reference(recordID: CKRecord.ID(recordName: primaryValueString, zoneID: referenceZoneID), action: .none)
              }
            case .int:
              if let primaryValueInt = object[primaryKey] as? Int, let obj = object as? CKRecordConvertible, !obj.isDeleted {
                let referenceZoneID = CKRecordZone.ID(zoneName: "\(object.objectSchema.className)sZone", ownerName: CKCurrentUserDefaultName)
                return CKRecord.Reference(recordID: CKRecord.ID(recordName: "\(primaryValueInt)", zoneID: referenceZoneID), action: .none)
              }
            default:
              return nil
            }
